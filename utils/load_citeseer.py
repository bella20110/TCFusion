...
def get_raw_text_citeseer(use_text=False, seed=0):
    if osp.exists(f"dataset/citeseer/citeseer_random_sbert.pt"):
        data = torch.load(f"dataset/citeseer/citeseer_random_sbert.pt", map_location='cpu')
    ...
    with open(citeseer_content, "r") as f:
        while True:
            ...
            if not any(lines):  # If all-MiniLM-L6-v2 lines are empty, end of file reached
                break
            idx_name = lines[0]
            text = lines[1]
            label_name = lines[2]
